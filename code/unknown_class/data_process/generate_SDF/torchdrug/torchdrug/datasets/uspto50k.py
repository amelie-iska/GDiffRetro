...
class USPTO50k(data.ReactionDataset):
    ...
    def __init__(self, path, outputpath, used_path, as_synthon=False, verbose=1, prepare_dataset=False, prepare_anchors=False, **kwargs): #, , table_path=None,
        ...
        if prepare_dataset and as_synthon:
            dataset = {"index":[], "molecule":[], "fragments":[], "linker":[], "product": []}
        ...
        for i in indexes:
            ...
            reactants, products = process_fn(reactant, product) # products = synthon || reactants = reactant
            ...
            if prepare_dataset and not as_synthon:  # for center computations here products is the initial molecule
                for k in range(len(reactants)):
                    ...
                    table.loc[table['product']==product_smi, "index"] = table.loc[table['product']==product_smi, "index"].apply(lambda x: x | {i})  #table.loc[table['product']==product_smi] = 
    ...
    def _get_difference(self, reactant, product):
        ...
        #print("edge_list in get difference", product.edge_list, len(product.edge_list))
    ...
    def split(self, ratios=(0.8, 0.1, 0.1)):
        ...
        for reaction in react2index:
            ...
            #print("react2index[reaction]", react2index[reaction])
