...
rdBase.rdkitVersion
print("molecule", rdBase.rdkitVersion)
...
class Molecule(Graph):
    ...
    def planar_face1(self): 
          """
          returns
            faces (list of nodes). the order corresponds to the edges
          """
          mol=self.to_molecule()
          lst=[]
    ...
    # def submolecule(self, index, compact=False):
    ...
    #     num_relation=len(edge_index)


    #     data_dict, meta_dict = self.data_mask(node_index=index,edge_index=edge_index)

    #     return type(self)(edge_list[edge_index], edge_weight=self.edge_weight[edge_index], num_node=len(index),
    #                       num_relation=self.num_relation, meta_dict=meta_dict, **data_dict)
    #     # return Molecule(edge_list[edge_index], self.atom_type[index], self.bond_type[edge_index],
    #     #            formal_charge=self.formal_charge[index], explicit_hs=self.explicit_hs[index],
    #     #            chiral_tag=self.chiral_tag[index], radical_electrons=self.radical_electrons[index], atom_map=self.atom_map[index],
    #     #            bond_stereo=self.bond_stereo[edge_index], stereo_atoms=self.stereo_atoms[edge_index],
    #     #            atom_feature=self.atom_feature[index], bond_feature=self.bond_feature[edge_index],
    #     #            num_node=len(index), num_relation=num_relation)
