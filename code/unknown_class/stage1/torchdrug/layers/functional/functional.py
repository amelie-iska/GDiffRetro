...
def variadic_sample_distribution(input, size, k):
    """
    Sample :math:`k` largest elements over sets with variadic sizes.

    Suppose there are :math:`N` sets, and the sizes of all sets are summed to :math:`B`.


    Parameters:
        input (Tensor): unnormalized distribution of shape :math:`(B, ...)`
        size (LongTensor): size of sets of shape :math:`(N,)`
        k (int or LongTensor): sample number

    Returns
        (LongTensor): their indexes of k samples for each set, (N, k)
    """

    cum_size = [0] + size.cumsum(0).tolist()
    sample_list = []
    for i in range(len(size)):
        input_i = input[cum_size[i]:cum_size[i+1]]
        sample_i = torch.multinomial(input_i, k, replacement=True)
