...
class EDM(torch.nn.Module):
    ...
    def forward(self, x, h, node_mask, fragment_mask, linker_mask, edge_mask, context=None):
        ...
        # Note: in our dataset, there are some graph without any linker to complete
        normalization = torch.where(
            normalization > 0, normalization, torch.full_like(normalization, self.n_dims + self.in_node_nf, dtype=torch.float)
        )
