...
def save_xyz_file(path, one_hot, positions, node_mask, names, is_geom, suffix=''):
    ...
    #const.GEOM_IDX2ATOM if is_geom else
    ...
    for batch_i in range(one_hot.size(0)):
        ...
        for atom_i in atom_idx:
            atom = atoms[atom_i].item() #%15 is weird but rare
            if atom>=16: print('15Error dictionary')
            atom = idx2atom[atom%16]
            f.write("%s %.9f %.9f %.9f\n" % (
...
def visualize_chain(
        path, spheres_3d=False, bg="black", alpha=1.0, wandb=None, mode="chain", is_geom=False, fragment_mask=None, output_gif=True
):
    files = load_xyz_files(path)
    save_paths = []

    # Fit PCA to the final molecule – to obtain the best orientation for visualization
    filter_file = [s for s in files if 'true' in s]
    positions, one_hot, charges = load_molecule_xyz(filter_file[0], is_geom=is_geom)
