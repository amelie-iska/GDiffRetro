...
from src.datasets import (
    ZincDataset, create_templates_for_linker_generation, get_dataloader, collate
)
...
class DDPM(pl.LightningModule):
    ...
    def __init__(
        ...
        dynamics_class = Dynamics  # if '.' in train_data_prefix else Dynamics
        ...
        # self.linker_size_sampler = DistributionNodes(LINKER_SIZE_DIST_final1)

    def setup(self, stage: Optional[str] = None):
        dataset_type = ZincDataset
        if stage == 'fit':
            self.is_geom = ('geom' in self.train_data_prefix) or ('MOAD' in self.train_data_prefix)
    ...
    def forward(self, data, training):
        x = data['positions']
        # h = data['one_hot']
        h = data['one_hot'] if not self.include_charges else torch.cat([data['one_hot'], data['charges']], dim=-1)
    ...
    def validation_epoch_end(self, validation_step_outputs):
        ...
        if (self.current_epoch + 1) % self.test_epochs == 0:
            sampling_results = self.sample_and_analyze(self.val_dataloader())  # visualization was here
    ...
    def sample_chain(self, data, sample_fn=None, keep_frames=None):
        ...
        h = template_data['one_hot'] if not self.include_charges else torch.cat(
            [template_data['one_hot'], template_data['charges']], dim=-1)
        ...
        if self.include_charges:
            chain = chain[:, :, :, :-1]  # remove the charge feature part
        x_, h_ = chain[:, :, :, :self.n_dims], chain[:, :, :, self.n_dims:]
        h_ = F.one_hot(torch.argmax(h_, dim=3), self.num_classes) * node_mask.unsqueeze(0)
        chain = torch.cat([x_, h_], dim=3)
