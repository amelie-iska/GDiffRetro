...
class Graph(core._MetaContainer):
    ...
    _meta_types = {"node", "edge", "graph", "node reference", "edge reference", "graph reference", "relation", "node_feature", "face"}
    ...
    def planar_face(self):
          """
          Use networkx
          returns
             list of faces
             each face is presented as set of nodes that belong to that face. 
             the order of nodes in those faces is preserved so that two neighbouring nodes are connected by some edge.
          """
          #1)adjacent list (dictionary) with sorted vertecies by polar angle
          edge_list1=self._edge_list[:,0:2].tolist()
          edge_list=self.edge_list
          G = nx.Graph()
          G.add_edges_from(edge_list1)
          G=G.to_directed()
          Y=nx.PlanarEmbedding(G)
          is_planar, embedding = nx.check_planarity(G)
          euklid_coo=nx.combinatorial_embedding_to_pos(embedding)
          adj={}  
          l=len(euklid_coo)
          for i, adjacency in G.adjacency():
               polar_coo={}
               adj[i]=[]
               x,y=euklid_coo[i]
               for j in list(G.nodes):
                    euklid_coo[j]=(euklid_coo[j][0]-x,euklid_coo[j][1]-y)

               for s in euklid_coo:
                  if s in adjacency:
                     polar_coo[s]=np.arctan2(euklid_coo[s][1], euklid_coo[s][0])
               polar_coo=dict(sorted(polar_coo.items(), key=lambda item: item[1]))
               for s in polar_coo:
                     if s!=i:
                       adj[i].append(s)
          #2)find the faces:
          ...
          for i in adj:
             while adj1[i]!=[]:
                 ...
                 d=b
                 boo=True
                 while (((a,b)!=(c,d))^(boo==True)):
                    boo=False
                    adj1[a].remove(b)    
                    index=adj[b].index(a)
                    if index+1!=len(adj[b]): 
                        next_vertex = adj[b][index+1]
                    else:
                         next_vertex = adj[b][0]
                    (a,b)=(b,next_vertex)
                    face.append(b)             
    ...
    def dual_graph(self):
         ...
         faces=self.planar_face1()
         num_node=len(faces)
         new_edge_list=torch.empty([0,3], dtype=torch.long, device=self.device)
         node_feature = torch.zeros(num_node, *self.node_feature.shape[1:],device=self.device)
         i=0 
         face_list={}                       
         for face in faces:
             l=len(face)
             face_list[i]=set()
             node_feature[i] = torch.mean(self.node_feature.float()[face], axis=0)
             for j in range(l-2):
                 face_list[i].add((face[j],face[j+1]))
                 face_list[i].add((face[j+1],face[j]))
             i+=1
         num_edge=0
         for i in range(num_node):
            for j in face_list[i].copy():
               for k in [x for x in range(num_node) if x != i]:
                  if j in face_list[k]:
                     (a,b)=j
                     index1=(edge_list[:,0:1]== torch.tensor([a])).nonzero(as_tuple=False).squeeze(1)[:,0]
                     index2=(edge_list[:,1:2][index1]== torch.tensor([b])).nonzero(as_tuple=False).squeeze(1)[:,0]
                     z=torch.tensor([[i,k, edge_list[index1][index2,2]],
                               [k,i, edge_list[index1][index2, 2]]],device=self.device)
                     num_edge =num_edge+2
                     new_edge_list=torch.cat((new_edge_list,z),0)                      
                     face_list[k]=face_list[k]-{(a,b),(b,a)}
                     face_list[i]=face_list[i]-{(a,b),(b,a)}
         for i in range(num_node):
            for j in face_list[i].copy():
                (a,b)=j
                index1=(edge_list[:,0:1]== torch.tensor([a])).nonzero(as_tuple=False).squeeze(1)[:,0]
                index2=(edge_list[:,1:2][index1]== torch.tensor([b])).nonzero(as_tuple=False).squeeze(1)[:,0]
                z=torch.tensor([[i,i, edge_list[index1][index2,2]]],device=self.device)
                num_edge =num_edge+1
                new_edge_list=torch.cat((new_edge_list,z),0) 
         #here we doubled self edges. however, the results for both versions are super close anyway

         return Graph(edge_list=new_edge_list, num_node=num_node, num_relation=self.num_relation, num_edge=num_edge, node_feature=node_feature)  
    ...
    """
    def visualize(self, title=None, save_file=None, figure_size=(30, 30), ax=None, layout="spring"):
    ...
    """            
    def visualize_graph(self, title=None, save_file=None, figure_size=(30, 30), ax=None, layout="spring"):
        """
...
class PackedGraph(Graph):
    ...
    def _get_offsets(self, num_nodes=None, num_edges=None, num_cum_nodes=None, num_cum_edges=None):
        #print("num_nodes1",num_nodes, num_nodes.size(),num_nodes.type())
        if num_nodes.size()==torch.Size([0]):
            ...
            if num_edges.size()==torch.Size([0]):
                    ...
                    #print("pizda", (num_cum_nodes - num_nodes).repeat_interleave(num_edges))
                    return (num_cum_nodes - num_nodes).repeat_interleave(num_edges)
                    #return torch.empty(0, dtype=torch.int64)

        if num_nodes is None:
            prepend = torch.tensor([0], dtype=torch.int64, device=self.device)
            #print("prepend", prepend)
            num_nodes = torch.diff(num_cum_nodes, prepend=prepend,dtype=torch.int64)

       
        if num_edges is None:
            prepend = torch.tensor([0], dtype=torch.int64, device=self.device)
            #print("prepend",prepend)
            num_edges = torch.diff(num_cum_edges, prepend=prepend,dtype=torch.int64)
        ...
        #print("num_cum_nodes",num_cum_nodes,num_cum_nodes.size(), num_cum_nodes.type())
        #print("num_nodes2",num_nodes, num_nodes.size(), num_nodes.type())
        #print("num_edges",num_edges,num_edges.size(),num_edges.type())
        #print("(num_cum_nodes - num_nodes).repeat_interleave(num_edges)",(num_cum_nodes - num_nodes).repeat_interleave(num_edges))
        return (num_cum_nodes - num_nodes).repeat_interleave(num_edges)
    ...
    def dual_graph(self):
      ...
      graphs=self.unpack()  # need to be modified not to unpack graph
      num_nodes=torch.empty([0], dtype=torch.long, device=self.device)
      num_edges=torch.empty([0], dtype=torch.long, device=self.device)
      edge_list2=torch.empty([0,3], dtype=torch.long, device=self.device)
      node_features=torch.empty([0], dtype=torch.long, device=self.device)
      
      for graph in graphs:
         edge_list1=graph._edge_list[:,0:2].tolist()
         edge_list=graph._edge_list
         faces=graph.planar_face1()
         num_node=torch.tensor([len(faces)],device=self.device)
         new_edge_list=torch.empty([0,3], dtype=torch.long, device=graph.device)
         node_feature = torch.zeros(num_node, *graph.node_feature.shape[1:],device=graph.device)
         i=0 
         face_list={}                        
         for face in faces:
             l=len(face)
             face_list[i]=[]
             node_feature[i] = torch.mean(graph.node_feature.float()[face], axis=0)
             for j in range(l-2):
                 face_list[i].append([face[j],face[j+1]])
                 face_list[i].append([face[j+1],face[j]])
             i+=1
         num_edge=torch.tensor([0],device=self.device)
         s=0
         for i in range(len(face_list)):
               while face_list[i]!=[]:
                   s=face_list[i][0]
                   face_list[i].remove(s)
                   [a,b]=s
                   face_list[i].remove([b,a])
                   for j in range(len(face_list)):
                      if (s in face_list[j]):
                         face_list[j].remove(s)
                         face_list[j].remove([b,a])
                         k=edge_list1.index(s)
                         if i!=j:
                             z=torch.tensor([[i,j, edge_list[k][2]],
                             [j,i, edge_list[k][2]]],device=graph.device)
                             num_edge =num_edge+2
                         else:
                             z=torch.tensor([[i,j, edge_list[k][2]]],device=graph.device)
                             num_edge =num_edge+1
                         edge_list = torch.cat((edge_list[:k],edge_list[k+1:]))
                         edge_list1.remove(s)
                         k=edge_list1.index([b,a])
                         edge_list = torch.cat((edge_list[:k],edge_list[k+1:]))
                         edge_list1.remove([b,a])
                         new_edge_list=torch.cat((new_edge_list,z),0) 
         num_nodes=torch.cat((num_nodes,num_node),0) 
         num_edges=torch.cat((num_edges,num_edge),0) 
         edge_list2=torch.cat((edge_list2,new_edge_list),0) 
         node_features=torch.cat((node_features,node_feature),0) 
      return PackedGraph(edge_list=edge_list2, num_nodes=num_nodes, num_relation=self.num_relation, num_edges=num_edges, node_feature=node_features) 
    ...
    def visualize_graph(self, titles=None, save_file=None, figure_size=(3, 3), layout="spring", num_row=None, num_col=None):
        """
