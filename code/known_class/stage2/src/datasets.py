...
def parse_molecule(known_class_num, mol, is_geom):
    one_hot = []
    charges = []
    atom2idx = const.ATOM2IDX
    charges_dict = const.CHARGES

    known_class_onehot = np.zeros(10)
    known_class_onehot[known_class_num-1] = 1
    known_class_onehot = np.array(known_class_onehot)

    for atom in mol.GetAtoms():
        one_hot.append(np.concatenate((get_one_hot(atom.GetSymbol(), atom2idx),known_class_onehot)))
...
class ZincDataset(Dataset):
    ...
    def preprocess(data_path, prefix, device):
        ...
        mapping_knowclass = torch.load('code/known_class/data_process/get_dataset_1st_stage/mapping_knownclass.pt')
        ...
        for (_, row), fragments, linker in generator:
            uuid = row['uuid']
            name = row['molecule']
            key_mflp = row['molecule'] + '&' + row['fragments'] + '&' + row['product'] + '&' + str(row['center1']) + '&' + str(row['center2']) +  '&' + str(row['anchors']) + '&' + str(row['linksize'])
            known_class_num = mapping_knowclass[key_mflp]
            frag_pos, frag_one_hot, frag_charges = parse_molecule(known_class_num, fragments, is_geom=is_geom)
            link_pos, link_one_hot, link_charges = parse_molecule(known_class_num, linker, is_geom=is_geom)
            ...
            if ''!=Chem.MolToSmiles(linker):
                ...
                positions = np.concatenate([frag_pos], axis=0)
                one_hot = np.concatenate([frag_one_hot], axis=0)
