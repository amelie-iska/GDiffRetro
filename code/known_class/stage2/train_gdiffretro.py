...
def main(args):
    ...
    torch_device = args.torch_device if args.device == 'gpu' else 'cpu'
    ...
    in_node_nf = number_of_atoms + args.include_charges + 10
    ## add for knownclass
    ...
    trainer = Trainer(
        ...
        devices=[int(str(args.torch_device)[5:])],
        # accelerator="gpu",
        # devices=[0,1,2,3,4],
...
if __name__ == '__main__':
    ...
    p.add_argument('--config', type=argparse.FileType(mode='r'), default='configs/uspto_gdiffretro.yml')
    #p.add_argument('--data', action='store', type=str,  default="datasets")
    p.add_argument('--data', action='store', type=str)
    ...
    p.add_argument('--last_checkpoint', type=str, default=None, help='Remove anchors context')
