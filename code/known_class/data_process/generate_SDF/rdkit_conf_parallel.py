...
def main(input_file, output_template, cores):
    
    ''' 
    smiles = []
    dataset = pd.read_csv(input_file)
    molecules = dataset["fragments"]
    [smiles.append(m) for m in molecules]  
    '''
    smiles = []
    ...
    assert cores < len(smiles)
    chunk_size = len(smiles) // cores
    print(chunk_size)
